...
<div style="height: auto;width: 100%;" class="all">
  <div class="site" v-for="site of ss" v-if="pa.indexOf('constructionSite') > 0||pa.indexOf('siteList') > 0" @click="$router.push({path:'/siteDetail',query:{id:site.id}})">
      ...
      window.onscroll = function () {
        ...
        if (scrollTop + windowHeight === scrollHeight) {
          if (_this.pa.indexOf('constructionSite') > 0) {
            _this.param.lastIndex = _this.ss.length
            _this.$ajax.get('/api/construction/site', {params: _this.param}).then((res) => {
