  ...
  setTimeout(
    function () {
      var S = 0; $.each($('.total'), function () {
        ...
        $(this).children('number').html(n1); S = S + s
      })
...
function hide () {
  if ($('.content').length == 0) {
    ...
    $('.bottom').eq(0).show(); $('.no').css('display', 'none')
  }
}
function sum () {
...
$(function () {
  ...
  $("ul input[type='checkbox']").change(function () {
    checkbox($(this)); var $ul_input = $(this).parents('ul').prev('.list').find('input')
    if ($(this).parents('ul').find("input[checked='checked']").length == $(this).parents('ul').children('li').length) {
      ...
      $ul_input.removeAttr('checked'); $ul_input.next('img').attr('src', 'img/c_checkbox_off.png')
    }
    sum()
  })
  $('.btn2').click(function () {
    if ($(this).next('.number').html() > 100) { $(this).next('.number').html(100); $('.alert').show().html('超出库存了！'); setTimeout(function () { $('.alert').hide() }, 2000); return false } else { $(this).prev('.number').html(parseInt($(this).prev('.number').html()) + 1) } total()
  })
  $('.btn1').click(function () {
  ...
  $('.number').click(function () {
    $('.text1').css({'display': 'flex', '-webkit-display': 'flex'}).attr({'ind': $(this).parents('li').index(), 'ind_1': $(this).parents('ul').attr('ind')}); $('.text1 input[type=number]').val($(this).html())
  })
  $('.text1 input[type="button"]').click(function () {
    if ($('.text1 input[type=number]').val() == '') { $('.alert').show().html('请输入数量！'); setTimeout(function () { $('.alert').hide() }, 2000); return false }
    if ($('.text1 input[type=number]').val() > 100) { $('.alert').show().html('超出库存了！'); setTimeout(function () { $('.alert').hide() }, 2000); return false }
    $('ul').eq($('.text1').attr('ind_1')).find('.number').eq($('.text1').attr('ind')).html($('.text1 input[type=number]').val()); $('.text1').css({'display': 'none', '-webkit-display': 'none'}); total()
  })
  $('.sett').click(function () {
